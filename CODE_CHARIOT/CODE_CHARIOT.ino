...
void loop()
{
  ...
  // Si le joystick est dans un sens, on tourne le moteur au maximum dans une direction, sinon on le ferme
  if(analogRead(JOYSTICK_Y) > 800){
    joystick_y = MOTEUR_Y_MAX;
  }
  if(analogRead(JOYSTICK_Y) < 200){
    joystick_y = MOTEUR_Y_MIN;
  } 
